    ...
    <div
        ...
        <main class="flex max-w-[335px] w-full flex-col-reverse lg:max-w-4xl lg:flex-row">
            <div
                ...
                @if(isset($company))
                    <div
                        class="mb-6 p-4 bg-[#FDFDFC] dark:bg-[#0a0a0a] rounded-sm border border-[#e3e3e0] dark:border-[#3E3E3A]">
                        <div class="flex items-start gap-4">
                            <img  alt="{{ $company->name }}"
                                src="{{ 'https://ui-avatars.com/api/?name=' . urlencode($company->name) . '&background=random&size=128' }}"
                                class="w-14 h-14 rounded-full object-cover flex-shrink-0">

                            <div class="flex-1">
                                <h2 class="text-sm font-medium mb-1">{{ $company->name }}</h2>
                                @if($company->handle)
                                    {{-- @dd($company) --}}
                                    <p class="text-[#706f6c] dark:text-[#A1A09A] mb-2">{{ $company->handle }}</p>
                                @endif
                            </div>
                        </div>
                    </div>
